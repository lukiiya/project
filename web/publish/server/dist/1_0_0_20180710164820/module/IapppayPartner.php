...
class IapppayPartner {
	...
	public function createPay($param) {
		...
		$iapppayConfig = $curEnvConfig->iapppayPartner->$source;
		...
		$payNotifyUrl = trim($curEnvConfig->iapppayPartner->payNotifyUrl);
		...
		//发送到爱贝服务后台请求下单
		$respData = request_by_curl($orderUrl, $reqData, '米多多彩票');
		//验签数据并且解析返回报文
		...
		if ($source == 'h5') {
			...
			$orderReq['app'] = $appid;
			$orderReq['url_r'] = $redirectUrl; //支付成功后支付回调URL地址
			$orderReq['url_h'] = $redirectUrl; //返回商户URL地址
			//组装请求报文   对数据签名
			$reqData = h5composeReq($orderReq, $appkey);
			$payUrl = $h5urlForPartner.$reqData;
		}
		$resp->data = $payUrl;
		$resp->errCode = 0;
		$resp->msg = "成功";
	...
	public function createBatchPay($param) {
		...
		$batchPayNotifyUrl = trim($curEnvConfig->iapppayPartner->batchPayNotifyUrl);
	...
	public function payNotify() {
		...
		$data = array(
			'userId' => $userId,
			'orderId' => $orderId,
			'tradeType' => 4,//交易方式, 0=爱贝, 1=支付宝, 2=威富通, 3=苹果支付, 4=爱贝合作
			'tradeNo' => $tradeNo,
			'tradeTime' => $tradeTime,
			'amount' => $amount
		);
