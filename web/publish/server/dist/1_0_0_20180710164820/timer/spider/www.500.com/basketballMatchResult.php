...
class Spider {
	...
	public function getMatchResult() {
		//http://live.m.500.com/home/lq/jclq/cur?render=local
		$time = time();
		$today = date("Y-m-d");
		$yesterday = date("Y-m-d", $time - 1*24*3600);
		$todayResult = $this->httpGet('http://ews.500.com/score/lq/info?vtype=jclq&expect='.$today.'&_t='.$time);
		$todayResult = json_decode($todayResult, true);
		$yesterdayResult = $this->httpGet('http://ews.500.com/score/lq/info?vtype=jclq&expect='.$yesterday.'&_t='.$time);
		...
		foreach($matchList as $match) {
			...
			//不要抓改期
			if ($status != 11 || empty($order) || empty($matchtime) || !key_exists('homescore', $match) || !key_exists('awayscore', $match)) {
				continue;
			}
	...
	public function updateMatchResult($resultMap) {
		...
		if (count($sqlArr) > 0) {
			...
			$database->multiFree();
			if ($result) {
				$this->common->logger->info('500彩票网篮球赛果更新成功');
			} else {
				$this->common->logger->info('500彩票网篮球赛果更新失败');
			}
		}
		$database->close();
		$this->common->logger->info('500彩票网篮球赛果更新完成');
	}
