...
class WuBaiDLT {
	...
	public function setIssuePrize() {
		...
		foreach ($issueArr as $issue) {
			$result = $this->httpGet('http://kaijiang.500.com/static/info/kaijiang/xml/dlt/'.$issue.'.xml?_A=PAXBKVNE'.time());
			...
			foreach ($xml->children() as $row) {
                ...
                } else if ($nodeName == 'ForeResult') {
					$ForeResult = trim($row->__toString());
				} else if ($nodeName == 'BackResult') {
					$BackResult = trim($row->__toString());
				}
			}
			...
			if (!preg_match('/^\d{2},\d{2},\d{2},\d{2},\d{2}$/', $ForeResult)) {
				$this->common->logger->info($this->lotteryName.'红球开奖号码有误');
				continue;
			}
			if (!preg_match('/^\d{2},\d{2}$/', $BackResult)) {
				$this->common->logger->info($this->lotteryName.'蓝球开奖号码有误');
				continue;
			}

			$drawNumber = $ForeResult.'+'.$BackResult;
