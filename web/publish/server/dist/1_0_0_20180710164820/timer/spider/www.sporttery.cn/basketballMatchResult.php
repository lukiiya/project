...
class Spider {
	...
	public function getMatchResultH5() {
		//http://info.sporttery.cn/wechat/bk_result_list.html
		$dataList = array();
		for ($i = 0; $i < 5; $i++) {
			$date = date("Y-m-d", time() - $i*24*3600);
			$result = $this->httpGet('http://i.sporttery.cn/open_v1_0/bk_match_list/get_bk_match_result/?username=11000000&password=test_passwd&date='.$date, $this->cookie);
		...
		foreach($dataList as $data) {
			...
			if (empty($number) || empty($home) || empty($away) || $beginTime <= 0 || empty($result) || $matchStatus != 'Final' || $sportteryMatchId <= 0 || ($poolStatus != 'Payout' && $poolStatus != 'Refund') || empty($week) || empty($num)) {
				continue;
			}
			$commonMatchId = '';
			//从周几判断出开售时间, 最多间隔7天
	...
	public function getMatchResultPC() {
		//获取赛果
		$result = $this->httpGet('http://info.sporttery.cn/iframe/match_result.php?type=bk&limit=500', $this->cookie);
		//过滤多余字符串
		...
		foreach($resultArr as $info) {
			...
			$result = trim($info[3]);
			$sportteryMatchId = (int)$info[5];
			$week = $this->weekMap[trim(mb_substr($number, 0, 2))];
			$num = trim(mb_substr($number, 2));
			if (empty($number) || empty($home) || empty($away) || $beginTime <= 0 || empty($result) || $sportteryMatchId <= 0 || empty($week) || empty($num)) {
				continue;
			}
			$commonMatchId = '';
			//从周几判断出开售时间, 最多间隔7天
	...
	public function updateMatchResult($resultMap) {
		...
		if (count($sqlArr) > 0) {
			...
			$database->multiFree();
			if ($result) {
				$this->common->logger->info('竞彩网篮球赛果更新成功');
			} else {
				$this->common->logger->info('竞彩网篮球赛果更新失败');
			}
		}
		$database->close();
		$this->common->logger->info('竞彩网篮球赛果更新完成');
	}
