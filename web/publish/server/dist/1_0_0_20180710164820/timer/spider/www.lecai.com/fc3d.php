...
class LecaiFC3D {
	...
	public function setIssuePrize() {
		...
		foreach ($issueArr as $issue) {
			$result = $this->httpGet('http://baidu.lecai.com/lottery/draw/ajax_get_detail.php?lottery_type=52&phase='.$issue);
			...
			$sjResult = $data['fc3d_sjh']['fc3d_sjh'];
			$sjBall = $sjResult['sjh'];
			if (empty($kjBall) || empty($sjBall)) {
			...
			//获取奖级
			$levelMap = array('prize1' => '直选', 'prize2' => '组三', 'prize3' => '组六');
			...
			if (empty($issue) || count($kjBall) != 3 || count($sjBall) != 3 || empty($drawNumber) || !is_array($prize) || count($prize) <= 0) {
