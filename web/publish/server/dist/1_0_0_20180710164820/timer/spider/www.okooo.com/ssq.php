...
class OkoooSSQ {
	...
	public function setIssuePrize() {
		...
		foreach ($issueArr as $issue) {
			$result = $this->httpGet('http://www.okooo.com/shuangseqiu/ssqkj/?LotteryNo='.$issue.'&Type=lotteryresult');
			...
			if (count($redBall) != 6 || count($blueBall) != 1) {
				continue;
			}
			$redBallArr = array();
			$blueBallArr = array();
			...
			//获取奖级
			$levelMap = array('1' => '一等奖', '2' => '二等奖', '3' => '三等奖', '4' => '四等奖', '5' => '五等奖', '6' => '六等奖');
