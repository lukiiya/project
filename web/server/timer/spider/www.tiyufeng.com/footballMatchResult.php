...
class Spider {
	...
	public function execute() {
		//获取赛果
		$result = file_get_contents('http://api.letoula.com/info/award/1?identity=com.tyf.letoula&platform=3');
		...
		foreach($resultArr as $info) {
            ...
            $halfHomeGoals = (int)$info['hostHalfScore'];
            $halfAwayGoals = (int)$info['guestHalfScore'];
			if (empty($commonMatchId) || empty($home) || empty($away) || $homeGoals < 0 || $awayGoals < 0 || $halfHomeGoals < 0 || $halfAwayGoals < 0) {
				continue;
			}
		...
		foreach ($matchList as $info) {
			...
			$field = array();
            $field[] = 'tiyufHalfResult="' . $database->escape($halfResult) . '"';
