...
class Spider {
	...
	public function getMatchResult() {
		...
		$result = file_get_contents('http://a.okooo.com/I/?method=mc.data.schedule.getOkoooSchedule&from=prize&LotteryType=SportteryScore&version=1.0.0.1&LotteryNo='.$date);
		...
		foreach($dataList as $data) {
			...
			if (empty($number) || empty($home) || empty($away) || $beginTime <= 0 || empty($result) || empty($halfResult) || $okoooMatchId <= 0 || empty($week) || empty($num)) {
				continue;
			}
			$commonMatchId = '';
			//从周几判断出开售时间, 最多间隔7天
	...
	public function getMatchResultMix() {
		...
		for ($i = 0; $i < 5; $i++) {
			...
			$result = file_get_contents('http://a.okooo.com/I/?method=mc.data.schedule.getOkoooSchedule&from=prize&LotteryType=SportterySoccerMix&version=1.0.0.1&LotteryNo='.$date);
			$result = json_decode($result, true);
			$code = (int)$result['code'];
			$codeStr = trim($result['code_str']);
			$info = $result['info'];
	...
	public function updateMatchResult($resultMap) {
		...
		foreach ($matchList as &$info) {
			...
			$field = array();
			$field[] = 'okoooHalfResult="' . $database->escape($halfResult) . '"';
		...
		if (count($sqlArr) > 0) {
			...
			$database->multiFree();
			if ($result) {
				$this->common->logger->info('澳客网足球赛果更新成功');
			} else {
				$this->common->logger->info('澳客网足球赛果更新失败');
			}
		}
		$database->close();
		$this->common->logger->info('澳客网足球赛果更新完成');
	}
