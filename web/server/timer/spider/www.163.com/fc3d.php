...
class WangYiFC3D {
	...
	public function setIssuePrize() {
		...
		$result = $this->httpGet('http://caipiao.163.com/t/awardlist.html?gameEn=x3d');
		...
		foreach ($list as $item) {
			...
			$kjNumber = trim($item['number']);
			$sjNumber = trim($item['extra']);
			if (!preg_match('/^\d{7}$/', $period) || !preg_match('/^\d \d \d$/', $kjNumber) || !preg_match('/^\d \d \d$/', $sjNumber)) {
				continue;
			}
			$kjNumberArr = explode(' ', $kjNumber);
			$sjNumberArr = explode(' ', $sjNumber);
			if (!is_array($kjNumberArr) || count($kjNumberArr) != 3 || !is_array($sjNumberArr) || count($sjNumberArr) != 3) {
				continue;
			}
			$drawNumberMap[$period] = implode(',', $kjNumberArr).'|'.implode(',', $sjNumberArr);
		}
