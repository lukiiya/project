...
if "%publishHistory%" == "yes" (
	cd ..\..\tools
	start grunt taskMaster --myTask=publish --myTarget=portal --myEnv=%1 --myDesc=%2 --myHistory=true
) else (
	if "%publishHistory%" == "no" (
		cd ..\..\tools
		start grunt taskMaster --myTask=publish --myTarget=portal --myEnv=%1 --myDesc=%2 --myHistory=false
	) else (
		echo.
