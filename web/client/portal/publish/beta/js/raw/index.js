...
if(/dev|debug/.test(ENV)) {
	...
	var PROXY = '/proxy/index.php';
	var UPLOAD_IMG_PATH = '/proxy/static/image/';
	var HASH_ENCODE = false;
	var CACHE_CGI = false;
	var MOCK_CGI = false;
} else if(ENV == 'beta') {
	var IMG_PATH = 'https://caifu-1251177394.cosgz.myqcloud.com/beta/shaimi/img/';
	var CSS_PATH = 'https://caifu-1251177394.cosgz.myqcloud.com/beta/shaimi/css/';
	var JS_PATH = 'https://caifu-1251177394.cosgz.myqcloud.com/beta/shaimi/js/';
	var OTHER_PATH = 'https://caifu-1251177394.cosgz.myqcloud.com/beta/shaimi/other/';
	var PROXY = '/cgi/index.php';
	var UPLOAD_IMG_PATH = location.protocol + "//" + location.host + '/cgi/static/image/';
	var HASH_ENCODE = false;
	var CACHE_CGI = true;
	var MOCK_CGI = false;
} else if(ENV == 'dist') {
	var IMG_PATH = 'http://caifu-1251177394.cosgz.myqcloud.com/dist/shaimi/img/';
	var CSS_PATH = 'http://caifu-1251177394.cosgz.myqcloud.com/dist/shaimi/css/';
	var JS_PATH = 'http://caifu-1251177394.cosgz.myqcloud.com/dist/shaimi/js/';
	var OTHER_PATH = 'http://caifu-1251177394.cosgz.myqcloud.com/dist/shaimi/other/';
	var PROXY = '/cgi/index.php';
	var UPLOAD_IMG_PATH = location.protocol + "//" + location.host + '/cgi/static/image/';
	var HASH_ENCODE = false;
	var CACHE_CGI = true;
...
(function (global, undefined) {
	...
	script.onload = script.onreadystatechange = function() {
		...
		var loadModule = [];
		if (ENV == "dev") {
			loadModule.push("external/vue");
			loadModule.push("external/jquery");
			loadModule.push("external/jweixin");
		...
		seajs.use(loadModule,function () {
			seajs.require('external/vue');
			seajs.require('external/jquery');
			seajs.require('external/jweixin');
			var common = seajs.require('module/common');
			var cache = trim(common.getUrlParam('cache'));
			if (cache == 'false') {
				common.clearCache();
			}
