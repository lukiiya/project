...
/**
 * 同步工具的配置类, 从文件中读取配置, 并进行检�? 配置为JSON格式
 * @author chengwu
 *
 */
public class Config {
    ...
    public Config(String localPath, String cosPath) {
        super();
        init(localPath, cosPath);
    }

    private void init(String localPath, String cosPath) {          
        ...
        //动态设置local_path和cos_path
        File directory = new File(localPath);
        localPath = directory.getAbsolutePath();
    	this.configJson.put("local_path", localPath);
